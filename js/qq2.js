var rule = {
    ...
    headers: {
        'User-Agent': 'Mozilla/5.0 (Linux; Android 5.1; TV Build/LMY47I) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.91 Safari/537.36'
    },
    timeout: 5000,
    ...
    lazy: '',
    // lazy:'js:input="https://cache.json.icu/home/api?type=ys&uid=292796&key=fnoryABDEFJNPQV269&url="+input.split("?")[0];log(input);var html=JSON.parse(request(input));log(html);input=html.url||input',
    ...
    二级: `js:
        ...
        var list = [];
        var QZOutputJson;
        var html = fetch(input, fetch_params);
        var sourceId = /get_playsource/.test(input) ? input.match(/id=(\\d*?)&/)[1] : input.split("cid=")[1];
        var cid = sourceId;
        var detailUrl = "https://v.%71%71.com/detail/m/" + cid + ".html";
        log("详情页:" + detailUrl);
        var pdfh = jsp.pdfh;
        var pd = jsp.pd;
        try {
        ...
        if (/get_playsource/.test(input)) {
            ...
            indexList.forEach(function(it) {
                var dataUrl = "https://s.video.qq.com/get_playsource?id=" + sourceId + "&plat=2&type=4&data_type=3&range=" + it + "&video_type=10&plname=qq&otype=json";
                eval(fetch(dataUrl, fetch_params));
                var vdata = QZOutputJson.PlaylistItem.videoPlayList;
            ...
            var url = "https://v.%71%71.com/x/cover/" + sourceId + ".html";
            if (video_lists.length === 1) {
                var vid = video_lists[0];
                url = "https://v.%71%71.com/x/cover/" + cid + "/" + vid + ".html";
                d.push({
                    title: "在线播放",
                    url: url
                })
                ...
                video_list.forEach(function(it, idex) {
                    var o_url = "https://union.video.qq.com/fcgi-bin/data?otype=json&tid=682&appid=20001238&appkey=6c03bbe9658448a4&union_platform=1&idlist=" + it.join(",");
                    var o_html = fetch(o_url, fetch_params);
                    eval(o_html);
                    QZOutputJson.results.forEach(function(it1) {
                        it1 = it1.fields;
                        var url = "https://v.%71%71.com/x/cover/" + cid + "/" + it1.vid + ".html";
                        var parts = it1.series_part_title.split(" ")[1];
    ...
    搜索: `js:
            ...
            var baseList = pdfa(html, "body&&.result_item_v");
            if (baseList.length > 0) {
                baseList.forEach(function(it) {
                    var longText = pdfh(it, ".result_title&&Text");
                    var shortText = pdfh(it, ".sub&&Text");
                    var url = pd(it, "div.result_item_v&&data-id");
                    var img = pd(it, ".figure_pic&&src");
                    var fromTag = pdfh(it, ".result_source&&Text");
                    var markAlt = pd(it, ".mark_v img&&alt");
                ...
                var baseLists = pdfa(html, "body&&.list_item");
                baseLists.forEach(function(it) {
                    var longText = pdfh(it, ".figure_title&&Text");
                    var shortText = pdfh(it, ".figure_desc&&Text");
                    var url = pd(it, ".figure_title&&a&&href");
                    var img = pd(it, ".figure_pic&&src");
                    var fromTag = pd(it, ".source_icon&&alt");
                    var parts = url.split('/');
                    var cid = parts[parts.length - 1].replace('.html', "");
                    if (!fromTag && longText) {
                        d.push({
                            title: longText,
                            img: img,
